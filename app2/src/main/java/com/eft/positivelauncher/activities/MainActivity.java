...
// Create an explicit intent for BasketActivity in the other app
                Intent intent = new Intent();
                intent.setComponent(new ComponentName("com.example.myapplication", "com.example.myapplication.ui.BasketActivity"));

// Add flags to start BasketActivity as a new task
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);

                try {
                    Log.i(TAG, "Launching BasketActivity...");
                    startActivity(intent);  // Start BasketActivity
                } catch (ActivityNotFoundException e) {
                    Log.e(TAG, "BasketActivity not found", e);
                }

// Fully exit the current app
                new Handler(Looper.getMainLooper()).postDelayed(() -> {
                    android.os.Process.killProcess(android.os.Process.myPid());
                    //System.exit(0);
                }, 500);  // Short delay to ensure transition
